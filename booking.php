...
$time=$_POST['time'];
$reason=$_POST['reason'];
if(isset($name)&&isset($lastname)&&isset($phone)&&isset($landmark)&&isset($address)&&isset($city)&&isset($pincode)&&isset($date)&&isset($time)&&isset($reason))
{
   $handel=mysql_connect("localhost","root","");
   if($handel)
   {
	mysql_select_db("water"); 
   $result=mysql_query("insert into booking values('$name','$lastname','$phone','$landmark','$address','$city','$pincode','$date','$time','$reason')");
   if($result)
   {
